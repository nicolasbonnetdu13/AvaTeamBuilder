...
public final class CommandMap {
    ...
    public boolean commandUser(User user, String command, Message message){
    	...
    	command = command.substring(1); // Retire l'espace en début de chaine pour une commande sur discord
        Object[] object = getCommand(command);
    ...
    private Object[] getCommand(String command){
        ...
        for(int i = 1; i < commandSplit.length; i++) {
        	//System.out.println("COMMANDSPLIT  ["+ (i-1)+ "] :" + commandSplit[i-1]);
        	args[i-1] = commandSplit[i];
        }
        SimpleCommand simpleCommand = commands.get(commandSplit[0]); 
        //System.out.println("TESTTTTTTESSSET" + commandSplit[0]);
